## Gene resource - encodes protein expression with regulatory control
...
@export var enzyme_id: String = ""  ## ID of the enzyme this gene produces

@export_group("Expression Parameters")
@export var basal_rate: float = 0.0001  ## Base expression rate (mM/s) without regulation
@export var max_expression_rate: float = 0.01  ## Upper limit on expression rate
@export var is_active: bool = true  ## Whether gene is being transcribed

@export_group("Regulation")
@export var activators: Array[RegulatoryElementData] = []
@export var repressors: Array[RegulatoryElementData] = []
...
func create_instance() -> GeneData:
	...
	for rep in repressors:
		instance.repressors.append(rep.create_instance())
...
func add_activator(molecule: String, kd: float = 1.0, max_fold: float = 10.0, hill: float = 1.0) -> RegulatoryElementData:
	var element = RegulatoryElementData.create_activator(molecule, kd, max_fold, hill)
	activators.append(element)
	emit_changed()
	return element

func add_repressor(molecule: String, kd: float = 1.0, max_fold: float = 10.0, hill: float = 1.0) -> RegulatoryElementData:
	var element = RegulatoryElementData.create_repressor(molecule, kd, max_fold, hill)
...
func get_summary() -> String:
	...
	return "%s → %s (%s, %d regulators)" % [gene_name, enzyme_id, status, reg_count]

func get_detailed_summary() -> String:
	var lines: Array[String] = []
	lines.append("Gene: %s → Enzyme: %s" % [gene_name, enzyme_id])
...
func _to_string() -> String:
	return "GeneData(%s → %s)" % [gene_name, enzyme_id]
