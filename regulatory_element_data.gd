## Regulatory element resource - modulates gene expression based on molecule concentration
## Uses Hill kinetics to model cooperative binding
class_name RegulatoryElementData
extends Resource
...
@export var type: Type = Type.ACTIVATOR
@export var molecule_name: String = ""  ## Name of the molecule this element responds to
@export var kd: float = 1.0  ## Dissociation constant (mM) - concentration for half-maximal effect
@export var max_fold_change: float = 10.0  ## Maximum fold change in expression
@export var hill_coefficient: float = 1.0  ## Cooperativity (n > 1 = positive cooperativity)

#endregion

#region Initialization

func _init(p_type: Type = Type.ACTIVATOR, p_molecule: String = "", p_kd: float = 1.0, p_max_fold: float = 10.0, p_hill: float = 1.0) -> void:
	type = p_type
...
static func create_activator(molecule: String, kd_val: float = 1.0, fold_change: float = 10.0, hill: float = 1.0) -> RegulatoryElementData:
	return RegulatoryElementData.new(Type.ACTIVATOR, molecule, kd_val, fold_change, hill)

static func create_repressor(molecule: String, kd_val: float = 1.0, fold_change: float = 10.0, hill: float = 1.0) -> RegulatoryElementData:
	return RegulatoryElementData.new(Type.REPRESSOR, molecule, kd_val, fold_change, hill)

## Create a runtime instance from this resource template
func create_instance() -> RegulatoryElementData:
	return duplicate(true) as RegulatoryElementData
...
func calculate_effect(molecules: Dictionary) -> float:
	...
	var mol: MoleculeData = molecules[molecule_name]
...
func _to_string() -> String:
	return "RegulatoryElementData(%s on %s)" % [get_type_string(), molecule_name]
