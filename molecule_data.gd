## Molecule resource - represents a chemical species with concentration and structure
## Saved as .tres files for reusable molecule libraries
class_name MoleculeData
extends Resource

#region Exported Properties (Inspector Editable)

@export var molecule_name: String = "Unnamed"
@export var concentration: float = 1.0:
	set(value):
		concentration = maxf(0.0, value)
		emit_changed()

@export var initial_concentration: float = 1.0
@export var potential_energy: float = 50.0  ## Energy stored in chemical bonds (kJ/mol)
@export var structural_code: Array[int] = []  ## Variable-length code representing molecular structure (0-9)
...
func _init(p_name: String = "", p_conc: float = 1.0, p_code: Array[int] = []) -> void:
	if p_name != "":
		molecule_name = p_name
...
func create_instance() -> MoleculeData:
	var instance = duplicate(true) as MoleculeData
	instance.concentration = initial_concentration
	instance._runtime_initialized = true
	return instance
...
static func generate_random_name() -> String:
	...
	var gen_name = PREFIXES[randi() % PREFIXES.size()]
	gen_name += MIDDLES[randi() % MIDDLES.size()]
	if randf() > 0.3:
		gen_name += SUFFIXES[randi() % SUFFIXES.size()]
	return gen_name
...
static func create_random(p_name: String = "", p_conc: float = -1.0) -> MoleculeData:
	var mol = MoleculeData.new()
	mol.molecule_name = p_name if p_name != "" else generate_random_name()
	mol.concentration = p_conc if p_conc >= 0.0 else randf_range(0.5, 5.0)
	mol.initial_concentration = mol.concentration
	mol.potential_energy = randf_range(20.0, 80.0)
	mol.structural_code = generate_random_code(randi_range(4, 8))
	return mol
...
func similarity_to(other: MoleculeData) -> float:
...
func get_summary() -> String:
	...
	return "%s: %.3f mM, E=%.1f kJ/mol, Code=%s%s" % [
		molecule_name, concentration, potential_energy, get_code_string(), lock_str
	]

func _to_string() -> String:
	return "MoleculeData(%s)" % molecule_name
